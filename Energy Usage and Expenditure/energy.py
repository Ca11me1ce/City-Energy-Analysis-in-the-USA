...
def data_cleanning(filename):
    ...
    with open(filename, "r") as f:
        ...
        findMiss(myDataFrame, output, filename)
        findInvaild(myDataFrame, output, filename)
        myDataFrame.to_csv("Clean_" + filename, index = False)
        f.close()
    output.write('\n')
    output.close()

def findMiss(myDataFrame, output, filename):
    output.write("For " + filename + ", find the fraction of missing values in each column:"+'\n')
...
def findInvaild(myDataFrame, output, filename):
    output.write("For " + filename + ", find the fraction of invalid values in each column:"+'\n')
    titles = myDataFrame.columns.values.tolist()  # Obtain the dataframe's column name
    i = 0
    for title in titles:
        if i > 1:
