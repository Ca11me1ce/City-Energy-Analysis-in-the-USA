...
#     Title:          Collect lmi housing units data
#     Command:        $python 
# ##################################################################
...
def get_data(api_key, f, zip_code, city, state):
    url='https://developer.nrel.gov/api/cleap/v1/lmi_housing_units'
    query_params={
        'city': city,
        'state_abbr': state,
        'zip': str(zip_code),
...
if __name__ == "__main__":
    ...
    f=open_file('../data_in_json/lmi_housing_units.json')
    ...
    j=0
    key=api_key[0]
    for i in range(10801, len(zip_ls)):
        try:
