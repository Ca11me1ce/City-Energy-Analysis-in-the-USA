...
#     Title:          Collect samll building pv data
#     Command:        $python 
# ##################################################################
...
def get_data(api_key, f, zip_code, city, state):
    url='https://developer.nrel.gov/api/cleap/v1/small_building_pv'
    query_params={
        'city': city,
        'state_abbr': state,
        'zip': str(zip_code),
...
if __name__ == "__main__":
    ...
    f=open_file('../data_in_json/small_building_pv.json')
    ...
    j=0
    key=api_key[0]
    for i in range(30279, len(zip_ls)):
        try:
