...
#     Title:          Collect building stock summaries data
#     Command:        $python 
# ##################################################################
...
def get_data(api_key, f, zip_code, city, state):
    url='https://developer.nrel.gov/api/cleap/v1/building_stock_summaries'
    query_params={
        'city': city,
        'state_abbr': state,
        'zip': str(zip_code),
...
if __name__ == "__main__":
    ...
    f=open_file('../data_in_json/building_stock_summaries.json')
    ...
    j=0
    key=api_key[0]
    for i in range(5347, len(zip_ls)):
        try:
