...
def exportIndustrialCSV(df, source):
    button = Button(label="Export Commercial Data CSV: Download", button_type="success")
    button.callback = CustomJS(args=dict(source=source), code=open(join(dirname(__file__), "download_commercial.js")).read())
    ...
    layout = row(controls)
    update(df, source)
    output_file('export_commercial_csv.html', title='Export CSV')
    show(layout)

if __name__ == "__main__":
    df = pd.read_csv(join(dirname(__file__), 'energy_commercial.csv'))
